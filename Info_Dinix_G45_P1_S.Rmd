...
Informe de Exploración Psicométrica de los puntajes de la prueba INDI obtenidas con muestra de Perú, Niveles 4-5. 

# Análisis Descriptivo Niveles 4-5
...
1. Grado de Instrucción Materna
1. Grado de Instrucción Paterna
...
```{r 45_Dem_QDOB, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
...
Los datos para el grupo de niveles 4-5 indica que en los meses de abril y agosto 2024 los subgrupos muestrales fueron relativamente homogéneos en tamaño. Sin embargo, en mayo y junio los subgrupos fueron bastante disparejos, con el grupo de mayo siendo el más pequeño (apenas algo más del 18%), mientras que el grupo de junio fue el más numeroso (casi el tercio de los participantes fue evaluado en  ese mes).
...
library(openxlsx)
library(lubridate)
INDI45 <- INDI45 %>% rename(`Fecha de Evaluación`= Fechin)
INDI45 <- INDI45 %>%
  mutate(across(c(02:04), as.factor))

INDI45 <- INDI45 %>%
  mutate(across(c(06:13), as.factor))

INDI45 <- INDI45 %>%
  mutate(across(c(20:43), as.factor))

t.Fechin <- tf(INDI45, `Fecha de Evaluación`)
...
grafiporc(
  ...
  pie_de_pagina = "Fuente: Grupo 4-5", 
  paleta_colores = "Dark2"
)
```
...
INDI45 <- INDI45 %>% rename(`Edad en Meses`= EDADMES)
# Generamos la tabla de frecuencias
t.agemo <- tf(INDI45, `Edad en Meses`)
...
AGEMO <- INDI45 %>%
  dplyr::select(`Edad en Meses`) %>%
  as.data.frame()


AGEMO$`Edad en Meses` <- as.numeric(AGEMO$`Edad en Meses`)
TED_agemo <- TED(AGEMO)
print(TED_agemo)

```

El promedio de edad fue de 24.68 meses, siendo la desviación estándar 7.32; debe tomarse en cuenta que este valor fue calculado en base al 100% de los casos en los que se contaba con la información (incluye casos "NA").
...
```{r 45_QDOB, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}

# Renaming the variable
INDI45 <- INDI45 %>% rename(Cuatrimestre= QDOB)
# tABLA DE fRECUENCIAS

t.QDOB <- tf(INDI45, Cuatrimestre)
...
```
Según comprobamos, la distribución de cuatrimestres es bastante pareja en la muestra. 
...
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Código Modular`= Codmod)
t.Codmod <- tfo(INDI45, `Código Modular`)
...
```{r 45_RegNat, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Re-etiquetar los valores de la columna "Categoria"

library(dplyr)

INDI45 <- INDI45 %>%
  mutate(Regnat = dplyr::recode(Regnat,
                                "1" = "Costa",
                                "2" = "Sierra",
                                "3" = "Selva"))
...
```
Existe un gran desbalance en la distribución porcentual de los integrantes según la región natural, sin embargo, esto es esperable ya que realmente refleja aproximadamente la distribución poblacional. 

## Área

A continuación, la información acerca del Área

```{r 45_Area, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Re-etiquetar los valores de la columna "Categoria"
INDI45 <- INDI45 %>%
...
```
Lo mismo que en el caso de las regiones naturales, la desproporción entre casos provenientes del área urbana y los provenientes del área rural no hace sino representar la realidad demográfica del país.

## Nivel Modalidad

A continuación, la información acerca del Nivel Modalidad

```{r 45_NivMod, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Nivel Modalidad`= Nivmod)
# Generamos tabla de frecuencias
t.NivMod <- tfo(INDI45, `Nivel Modalidad`)
...
En este caso, como es aparente, la abrumadora mayoría de participantes (más del 84%) provienen del nivel Inicial-Jardín. Esto probablemente refleja la proporción existente en la población general. 

## Gestión

A continuación, la información acerca de la Gestión (Pública o Privada) de la institución edicativa.

```{r 45_Gest, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(Gestión= Gest)
# Recodificando los valores de la variable 
INDI45 <- INDI45 %>%
  mutate(Gestión = dplyr::recode(Gestión,
                            "1" = "Pública",
                            "2" = "Privada"))
# Generamos tabla de frecuencias ordenadas
t.Gest <- tfo(INDI45, Gestión)
...
De forma similar a lo que se da en la mayoría de variables demográficas en las que existe una aparente desproporción, esta simplemente refleja la realidad nacional que la muestra intenta representar. 

## Departamento

A continuación, la información acerca del Departamento en donde vive el participante.

```{r 45_Depa, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}

# Renaming the variable
INDI45 <- INDI45 %>% rename(Departamento= Reg)
INDI45$Departamento <- as.factor(INDI45$Departamento)

t.Depa <- tfo(INDI45, Departamento)
...
Los departamentos de Piura y Lima son los que aportan la mayor cantidad de participantes (casi 60% de toda la muyestra general entre ambos). Siempore reflejando la proporción poblacional, el departamento de Loreto es el que aporta la manor cantidad de participantes,
...
```{r 45_Quintus, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}

INDI45$Quintil <- as.factor(INDI45$Quintil)

t.Quin <- tf(INDI45, Quintil)
...
El indicador de pobreza nos evidencia aquí que más del 60% de la muestra proviene de los quintiles de mayor ingreso (el 40% de la población con mayores ingresos), mientras que menos del 20% de la muestra proviene del 40% de la población más pobre.

## Grado de Instrucción Materna

```{r 30_Grainsmad, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
INDI45$Grainsmad <- as.factor(INDI45$Grainsmad)

t.Quin <- tf(INDI45, Grainsmad)
graficar_porcentajes_horizontal <- function(variable, data, 
                                            titulo = "Distribución porcentual", 
                                            etiqueta_x = "Porcentaje", 
                                            etiqueta_y = "Categorías", 
  ...
  # Crear el gráfico de barras con porcentajes en sentido horizontal y borde negro
  ggplot(datos_porcentajes, aes(x = {{ variable }}, y = porcentaje, fill = {{ variable }})) +
    geom_col(width = 0.75, colour = "black") + # Agregar borde negro
    geom_text(aes(label = paste0(porcentaje, " %")), hjust = -0.2) + # Ajustar posición horizontal de las etiquetas
    scale_fill_brewer(palette = paleta_colores) +
    labs(
      title = titulo,
      x = etiqueta_x,
...
# Ejemplo de uso
# Supongamos que `INDI45` es tu conjunto de datos y `Grainsmad` es la variable categórica.
...
# Ejemplo de uso
# Supongamos que `INDI45` es tu conjunto de datos y `Grainsmad` es la variable categórica.
graficar_porcentajes_horizontal(
  variable = Grainsmad, 
   data = INDI45, 
   titulo = "Distribución Porcentual Nivel Educ. Materno", 
   etiqueta_x = "Porcentaje", 
   etiqueta_y = "Categorías", 
   pie_de_pagina = "Fuente: Grupo 4-5", 
   paleta_colores = "BuPu")
...
#   variable = Grainsmad, 
#   data = INDI45, 
#   titulo = "Distribución Porcentual de Participantes según Grado Educativo Materno", 
#   etiqueta_x = "Educ. Materna", 
#   etiqueta_y = "Porcentaje de Participantes", 
#   pie_de_pagina = "Fuente: Grupo 4-5", 
#   paleta_colores = "Dark2"
# )
...
## Grado de Instrucción Paterna

```{r 30_Grainspad, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
INDI45$Grainspad <- as.factor(INDI45$Grainspad)

t.Quin <- tf(INDI45, Grainspad)
graficar_porcentajes_horizontal <- function(variable, data, 
                                            titulo = "Distribución porcentual", 
                                            etiqueta_x = "Porcentaje", 
                                            etiqueta_y = "Categorías", 
...
# Ejemplo de uso
# Supongamos que `INDI45` es tu conjunto de datos y `Grainspad` es la variable categórica.
...
graficar_porcentajes_horizontal(
  variable = Grainspad, 
   data = INDI45, 
   titulo = "Distribución Porcentual Nivel Educ. Paterno", 
   etiqueta_x = "Porcentaje", 
   etiqueta_y = "Categorías", 
   pie_de_pagina = "Fuente: Grupo 4-5", 
...
```{r 45_VSS, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(Incidencia = VSS)
# INDI45 <- INDI45 %>%
#   mutate(Incidencia = as.character(Incidencia))
INDI45 <- INDI45 %>%
  mutate(Incidencia = dplyr::recode(Incidencia,
                                    "0" = "No",
                                    "1" = "Sí"))

t.VSS <- tf(INDI45, Incidencia)
...
```{r 45_VSSdesper, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Incidencia Perinatal` = VSSdesper)
INDI45 <- INDI45 %>%
  mutate(`Incidencia Perinatal` = dplyr::recode(`Incidencia Perinatal`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSdesper <- tf(INDI45, `Incidencia Perinatal`)
...
Este cuadro nos indica que tan solo uno de cada diez participantes ha sufrido alguna incidencia, entre las consideradas en el estudio. 

## Incidencia Tratamiento Médico (VSSttomed)

A continuación, la información acerca de la Incidencia por Tratamiento Médico.

```{r 45_VSSttomed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. Médico` = VSSttomed)
INDI45 <- INDI45 %>%
  mutate(`Tto. Médico` = dplyr::recode(`Tto. Médico`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSttomed <- tf(INDI45, `Tto. Médico`)
...
```{r 45_VSSpatolo, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Patología` = VSSpatolo)
INDI45 <- INDI45 %>%
  mutate(Patología = dplyr::recode(Patología,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSpatolo <- tf(INDI45, Patología)
...
```{r 45_VSSnegl, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(Negligencia = VSSnegl)
INDI45 <- INDI45 %>%
  mutate(Negligencia = dplyr::recode(Negligencia,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSnegl <- tf(INDI45, Negligencia)
...
```{r 45_VSSmud, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Inc. Mudanza` = VSSmud)
INDI45 <- INDI45 %>%
  mutate(`Inc. Mudanza` = dplyr::recode(`Inc. Mudanza`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSmud <- tf(INDI45, `Inc. Mudanza`)
...
```{r 45_VSSconsus, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Inc. Consumo` = VSSconsus)
INDI45 <- INDI45 %>%
  mutate(`Inc. Consumo` = dplyr::recode(`Inc. Consumo`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSconsus <- tf(INDI45, `Inc. Consumo`)
...
```{r 45_VSSdesemp, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Inc. por Desempleo` = VSSdesemp)
INDI45 <- INDI45 %>%
  mutate(`Inc. por Desempleo` = dplyr::recode(`Inc. por Desempleo`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSdesemp <- tf(INDI45, `Inc. por Desempleo`)
...
```{r 45_VSSfamprilib, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Inc. Familiar` = VSSfamprilib)
INDI45 <- INDI45 %>%
  mutate(`Inc. Familiar` = dplyr::recode(`Inc. Familiar`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSfamprilib <- tf(INDI45, `Inc. Familiar`)
...
```{r 45_VSSotro, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Inc. Otros` = VSSotro)
INDI45 <- INDI45 %>%
  mutate(`Inc. Otros` = dplyr::recode(`Inc. Otros`,
                            "0" = "No",
                            "1" = "Sí"))
t.VSSotro <- tf(INDI45, `Inc. Otros`)
...
```{r 45_RTT, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. Técnico` = RTT)
INDI45 <- INDI45 %>%
  mutate(`Tto. Técnico` = dplyr::recode(`Tto. Técnico`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTT <- tf(INDI45, `Tto. Técnico`)
...
```{r 45_RTTasipsi, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. Psicológico` = RTTasipsi)
INDI45 <- INDI45 %>%
  mutate(`Tto. Psicológico` = dplyr::recode(`Tto. Psicológico`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTasipsi <- tf(INDI45, `Tto. Psicológico`)
...
```{r 45_RTTasipsiq, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. Psiquiátrico` = RTTasipsiq)
INDI45 <- INDI45 %>%
  mutate(`Tto. Psiquiátrico`  = dplyr::recode(`Tto. Psiquiátrico` ,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTasipsiq <- tf(INDI45, `Tto. Psiquiátrico` )
...
```{r 45_RTTasiped, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. Pedagógico` = RTTasiped)
INDI45 <- INDI45 %>%
  mutate(`Tto. Pedagógico` = dplyr::recode(`Tto. Pedagógico`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTasiped <- tf(INDI45, `Tto. Pedagógico`)
...
```{r 45_RTTasipsim, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
INDI45 <- INDI45 %>% rename(`Tto. Psicomotriz` = RTTasipsim)
INDI45 <- INDI45 %>%
  mutate(`Tto. Psicomotriz` = dplyr::recode(`Tto. Psicomotriz`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTasipsim <- tf(INDI45, `Tto. Psicomotriz`)
...
```{r 45_RTTasifon, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. Fonoaudiológico` = RTTasifon)
INDI45 <- INDI45 %>%
  mutate(`Tto. Fonoaudiológico` = dplyr::recode(`Tto. Fonoaudiológico`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTasifon <- tf(INDI45, `Tto. Fonoaudiológico`)
...
```{r 45_RTTdifdiag, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. por Dif. Diag.` = RTTdifdiag)
INDI45 <- INDI45 %>%
  mutate(`Tto. por Dif. Diag.`  = dplyr::recode(`Tto. por Dif. Diag.` ,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTdifdiag <- tf(INDI45, `Tto. por Dif. Diag.` )
...
```{r 45_RTTdisc, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Tto. por Discapacidad` = RTTdisc)
INDI45 <- INDI45 %>%
  mutate(`Tto. por Discapacidad` = dplyr::recode(`Tto. por Discapacidad`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTdisc <- tf(INDI45, `Tto. por Discapacidad`)
...
## Resumen de Indicadores y Tratamientos

A continuación, un cuadro que resume la proporción de incidencias y tratamientos:
...
# # Asegurarse de que las columnas seleccionadas son numéricas y solo contienen 0 y 1
# INDI45[, 23:30] <- INDI45[, 23:30] %>%
#   mutate(across(everything(), ~ as.numeric(. %in% c(1, "1"))))
#
# INDI45[, 33:39] <- INDI45[, 33:39] %>%
#   mutate(across(everything(), ~ as.numeric(. %in% c(1, "1"))))
library(dplyr)
...
# tabla_grafico1 <- analizar_dicotomicas_numericas(data = INDI45,
#         rango_indices = c(23:30),
#         titulo_tabla = "Tabla de Frecuencia de Incidencias",
#         titulo_grafico = "Porcentajes de Incidencias",
#         etiqueta_x = "Incidencias",
...
#
# # Guardar el objeto
# save(tabla_grafico1, file = "tabla_grafico1.RData")
#
#
# # Ejecutar la función y guardar el resultado
# tabla_grafico2 <- analizar_dicotomicas_numericas(data = INDI45,
#         rango_indices = c(33:39),
#         titulo_tabla = "Resumen de Tratamientos",
#         titulo_grafico = "Porcentajes de Tratamiento",
#         etiqueta_x = "Tratamiento",
...
#
# # Guardar el objeto
# save(tabla_grafico2, file = "tabla_grafico2.RData")
load("tabla_grafico1.RData")
print(tabla_grafico1)
load("tabla_grafico2.RData")
print(tabla_grafico2)
```


Los cuadros anteriores nos indican que, en cuanto a incidencias, el rubrop "Otros" es el más frecuente, seguido de la Incidencia por desempleo y Mudanza; entre los Tratamientos, el Psicológico y el Fonoaudiológico fueron los m´ñas frecuentes. 
...
```{r 45_Insant3a, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Instruc. Previa` = Insant3a)
INDI45 <- INDI45 %>%
  mutate(`Instruc. Previa`= dplyr::recode(`Instruc. Previa`,
                            "0" = "No",
                            "1" = "Sí",
                            "2" = "NS/NR"))
t.Insant3a<- tf(INDI45, `Instruc. Previa`)
...
Comprobamos que casi el 60% de los participantes no recibieron instrucción previa al  Nivel 3.

# Análisis Escalar Nivel 4-5


En esta sección pasaremos a analizar la escala INDI en su aplicación al grupo de Nivel 4-5. 

## Estructura de Subescalas

1. La subescala "C" está compuesta por los ítems C1 a C26 (26 ítems)
1. La subescala "M" está compuesta por los ítems M1 a M06 (06 ítems)
1. La subescala "S" está compuesta por los ítems S1 a S14 (14 ítems)
...
```{r 45_TED_SUBESCALAS, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
...
INDI45_i <- invertir_y_sumar(
  df = INDI45,
  indices_inversos = c(78:81, 85:89),  # Ítems inversos
  nombre_variable_sumatoria = "SSUM",  # Nombre de la variable sumatoria
  posicion_variable = 105  # Ubicación deseada
)
...
# Ejemplo: Convertir columnas de la 3 a la 6 en un dataframe a numéricas
INDI45_i[ , 44:95] <- lapply(INDI45_i[ , 44:95], as.numeric)
INDI45_i[ , 103:106] <- lapply(INDI45_i[ , 103:106], as.numeric)
# Llamar a la función y mostrar los resultados
LIMPION <- ayudin(dataframe = INDI45_i, columnas = c(44:106))
...
```{r 45_Histos, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
library(ggplot2)
library(dplyr)
library(RColorBrewer)
...
```{r 45_QQ, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
...
```{r 45_Norma, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
...
En resumen, los cuadros y tablas anteriores nos indican que ninguna de las subescalas del INDI evidencian normalidad; por el contrario, existen fuertes indicios de la existencia outiliers, sobre todo en SSUM y DSUM.
...
```{r 45_alpha, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
De hecho, podría decirse que algunas subescalas evidencian una confiabilidad demasiado alta, específicamente hablamos de la Escala "C". Recomendamos analizar con el mayor cuidado la posibilidad de detectar ítems redundantes o que prrobablemente midan prácticamente los mismos aspectos más de una vez.

Asimismo, notamos que suprimir el ítem D3 podría incrementar (si bien ligeramente) la confiabilidad global del instrumento. Aun así, es solo un dato psicométrico; correspondería a los diseñadores y usuarios directos del instrumento tomar o no esa decisión, tomando en cuenta aspectos adicionales, como el contenido del ítem y su relevancia en vista de los objetivos del instrumento.
...
```{r 45_AFC, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
library(lavaan)
