...
```{r 45_RTTnin, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results = 'asis'}
# Renaming the variable
INDI45 <- INDI45 %>% rename(`Ningún Tto.` = RTTnin)
INDI45 <- INDI45 %>%
  mutate(`Ningún Tto.` = recode(`Ningún Tto.`,
                            "0" = "No",
                            "1" = "Sí"))
t.RTTnin <- tfo(INDI45, `Ningún Tto.`)
...
# Llamar a la función y mostrar los resultados
# pre_TED_C3 <- ayudin(dataframe = INDI45, columnas = c(94:103))
...
```{r 45_Outliers, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
INDI45_REDUX <- INDI45 %>% select(Codigo, Regnat, Departamento) # Extraigo los campos categóricos que me interesan del df original
# Realizar el left join
LIMPIO <- left_join(PRELIMPIO, INDI45_REDUX, by = "Codigo")
...
```
Para resumir, podemos ofrecer las tablas siguientes:

```{r 45_Outliers_Resumen}
library(dplyr)
library(knitr)
...
items_intensos_resumen <- function(data, rango_columnas, columna_contexto, multiplicador_IQR = 1.5) {
  ...
  # Inicializar un data frame para registrar los outliers
  ...
  for (columna in columnas_a_evaluar) {
    ...
    # Registrar los casos de outliers
    if (length(filas_outliers) > 0) {
      ...
      registros_outliers <- rbind(registros_outliers, registros)
    }
  }
  
  # Generar la matriz de doble entrada
  if (nrow(registros_outliers) > 0) {
    matriz_doble_entrada <- registros_outliers %>%
      group_by(Columna, Valor_Outlier) %>%
      summarise(Frecuencia = n(), .groups = "drop")
    
    # Generar la tabla de frecuencias del campo Contexto
    tabla_frecuencias <- registros_outliers %>%
      count(Contexto, sort = TRUE, name = "Frecuencia")
    
    # Imprimir la matriz de doble entrada con kable
    cat("### Matriz de doble entrada\n")
    kable(matriz_doble_entrada, format = "markdown", row.names = FALSE)
    
    # Imprimir la tabla de frecuencias con kable
    cat("\n### Tabla de frecuencias descendentes\n")
    kable(tabla_frecuencias, format = "markdown", row.names = FALSE)
  } else {
    cat("No se detectaron valores atípicos en las columnas evaluadas.\n")
  }
}
