...
```{r 30_P2_AFC_Indepth, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
INDI30_i[ , 43:92] <- lapply(INDI30_i[ , 43:92], as.numeric)

# Llamar a la función y mostrar los resultados
LIMPION <- ayudin(dataframe = INDI30_i, columnas = c(43:103))
...
1. Los ítems de la subescala "S" se muestran ya invertidos. Originalmente, los ítems S1, S2, S7, S8 y S9 se comportan como ítems directos (a mayor puntaje en el ítem, mayor valor o presencia de la variable a medir), mientras que los ítems S3, S4, S5, S6, S10, S11, S12, y S13 se comportan como ítems inversos. Luego de la conversión, los ítems originalmente inversos de la Escala "S" ya no muestran cargas factoriales negativas, aunque ninguno evidencia cargas factoriales aceptables. 
1. Entre los ítems de la subescala "D", casi todos ostentan buenas cargas factoriales (siendo además todos ítems directos).


Ya desde el punto de vista conceptual, podemos pasar a analizar el *contenido* de los ítems en cuestión. En especial, nos centraremos en los ítems "problemáticos". 

```{r 30_P2_Escalas_Descritas, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}



library(readxl)
infovar <- read_excel("infovar30.xlsx")
...
* Los ítems inversos de la subescala "S" parecen efectivamente medir la variable latente en sentido opuesto. Los ítems originalmente inversos muestran también cargas factoriales relativamente bajas. En especial, S3 ("Evita relacionarse con otros en diferentes situaciones"); S10 ("Se muestra tímido y/o no comparte sus experiencias con el grupo"); y S11 ("Tiene un aspecto triste, cansado y/o preocupado"). Las razones de esta relativa ineficiencia de los ítems inversos de la subescala "S" puede deberse o bien a aspectos de contenido (los ítems no son adecuadamente interpretados o quizás haya un efecto cultural/lingüístico), o a aspectos metodológicos (puede que haya existido un efecto debido a la secuencia de administración).
* Los ítems de la subescala "D" parecen reflejar apropiadamente la dimensión a evaluar.  
...
```{r 30_P2_Outliers, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
INDI30_CATEGO$Regnat <- factor(INDI30_CATEGO$Regnat, 
                            levels = c("1", "2", "3"),  # Valores originales
                            labels = c("Costa", "Sierra", "Selva"))  # Nuevas etiquetas
# Cambiar la variable "Regnat" a factor y reetiquetar sus valores
INDI30_CATEGO$Reg <- factor(INDI30_CATEGO$Reg)  # Nuevas etiquetas
 
PRELIMPIO <- ayudin(dataframe = INDI30_i, columnas = c(1, 43:103)) # Extraigo solo valores no NA y numéricos del df original
# Realizar el left join
LIMPIO <- left_join(PRELIMPIO, INDI30_CATEGO, by = "Codigo")
...
items_intensos(
  ...
  rango_columnas = 2:51,     # Rango de columnas numéricas a evaluar (columnas 2 a 4)
...
```{r 30_Outliers_Resumen, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
Las conclusiones apuntan a que la única escala que contiene valores extremos en la escala S, en especial los ítems S6 y S12. Hay que señalar que los parámetros elegidos para definir "valor extremo" (tres veces el rango intercuartílico, en lugar del nivel usual de rango y medio) son extremos en verdad; la distribución de los puntajes es bastante dispersa.
...
Finalmente, el total de los casos con valores extremos (51 casos) no llega a sobrepasar el 5% del total de la muestra, por lo que se considera que no valdría la pena tomar otras medidas de gestión de valores extremos, lo cual llevaría necesariamente al recálculo de todos los índices.

```{r 30_P2_Resumen_Binarios, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results='asis'}
...
# resumen_binario(INDI30, columnas = c(22:30), equivalencias = c("1","0"),
...
#                             color_borde = "black")
# 
# resumen_binario(INDI30, columnas = c(33:39), equivalencias = c("1","0"),
#                             titulo_grafico = "Distribución de Respuestas 'Sí'",
#                             etiqueta_x = "Respuestas Sí",
#                             etiqueta_y = "Porcentaje",
#                             pie_de_pagina = "Fuente: Nivel 4-5",
...
```{r 30_P2_Transformaciones_Log, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
```{r 30_P2_Transformaciones_BoxCox, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
```{r 30_P2_Inversión_Items, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results='asis'}
...
LIMPIO$S3_i <- 6 + 1 - LIMPIO$S3 #ok
LIMPIO$S4_i <- 6 + 1 - LIMPIO$S4 #ok
LIMPIO$S5_i <- 6 + 1 - LIMPIO$S5 #ok
LIMPIO$S6_i <- 6 + 1 - LIMPIO$S6 #ok
LIMPIO$S10_i <- 6 + 1 - LIMPIO$S10 #ok
LIMPIO$S11_i <- 6 + 1 - LIMPIO$S11 #ok
LIMPIO$S12_i <- 6 + 1 - LIMPIO$S12 #ok
LIMPIO$S13_i <- 6 + 1 - LIMPIO$S13 #ok
...
LIMPIO$SSUM <- rowSums(LIMPIO[, c(34:45)]) # Escala S Sumatoria, tal cual
LIMPIO$B_SSUM <- rowSums(LIMPIO[, c(34:35, 64:67, 68:71)]) # Escala S Sumatoria, con los ítems inversos
...
```{r 30_P2_Discriminante, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}

# Vamos a generar un DF con la data necesaria
# Excluir columnas por índice
# DISCRI <- LIMPIO [, -c(35:38,42:45, 52:58, 62:63, 72)]  # Excluir las columnas que corresponden a los ítems inversos.

# Reordenamos las columnas
# DISCRI <- DISCRI %>%
#   relocate(D3_i, .before = D4)
# DISCRI <- DISCRI %>%
#   relocate(c(47:50), .before = c(35))
# DISCRI <- DISCRI %>%
#   relocate(c(51:54), .before = c(42))
# DISCRI <- DISCRI %>%
#   relocate(D_sum, .after = last_col())
# DISCRI <- DISCRI %>%
...
Trump <- function(nombre_escala, df_final, df_inicial, prefijo, rango_columnas) {
  ...
  df_final <- df_inicial[order(df_inicial$C_sum, decreasing = TRUE), ]
...
infovar_C <- infovar[1:23, ]
infovar_M <- infovar[24:31, ]
infovar_S <- infovar[32:44, ]
infovar_D <- infovar[45:50, ]
# # Llamar a la función
# Tab_Discri_M <- Trump(
...
```{r 30_P2_Discriminante_C, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
# Llamar a la función
# Llamar a la función
tabla <- Trump(
  ...
  df_inicial = LIMPIO, 
  prefijo = "C",
  rango_columnas = 2:24  # Columnas de los ítems
)
```
...
```{r 30_P2_Discriminante_M, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
# Llamar a la función
Tab_Discri_M <- Trump(
  nombre_escala = "Escala M",
  df_final = DISCRIM,
  df_inicial = LIMPIO,
  prefijo = "M",
  rango_columnas = 25:32  # Especificar el rango de columnas para los ítems
)
```

* En general, la Escala M contiene ítems con buenos niveles de discriminación.
* Tan solo los ítems M1 y M3 tienen un nivel menor de discriminación, de todos modos está dentro de un rango más que aceptable.


## Conclusiones Escala S

```{r 30_P2_Discriminante_S, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
# Llamar a la función
Tab_Discri_S <- Trump(
  nombre_escala = "Escala S",
  df_final = DISCRIS,
  df_inicial = LIMPIO,
  prefijo = "S",
  rango_columnas = 33:45  # Especificar el rango de columnas para los ítems
)
```

* En general, tan solo los ítems S1, S2, S7, S8 y S9 tienen un buen nivel discriminación. El resto de ítems (todos ellos originalmente inversos) ofrecen un nivel discriminante francamente malo. 
...
```{r 30_P2_Discriminante_D, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
# Llamar a la función
Tab_Discri_D <- Trump(
  nombre_escala = "Escala D",
  df_final = DISCRID,
  df_inicial = LIMPIO,
  prefijo = "D",
  rango_columnas = 46:51  # Especificar el rango de columnas para los ítems
)
```

* En general, los ítems que componen la Escala D encuentra dentro de niveles aceptables. Los ítems ofrecen un nivel discriminante entre bueno y excelente.
