...
Informe de Exploración Psicométrica de los puntajes de la prueba DINI obtenidas con muestra de Perú, Nivel 3. 
...
A continuación veamos la distribución según las fechas de evaluación:
Para este Nivel, podemos apreciar que la recolección datos fue ligeramente menos numerosa en los dos
primeros meses (Abril y Mayo) comparada con los dos meses subsiguientes (Junio y Agosto)
...
# Supongamos que tu dataframe se llama INDI30 y tu variable es Fechin
...
INDI30$Grainsmad <- as.factor(INDI30$Grainsmad)

t.Quin <- tf(INDI30, Grainsmad)
...
# Ejemplo de uso
# Supongamos que `INDI30` es tu conjunto de datos y `Grainsmad` es la variable categórica.
...
INDI30$Grainspad <- as.factor(INDI30$Grainspad)

t.Quin <- tf(INDI30, Grainspad)
graficar_porcentajes_horizontal <- function(variable, data, 
                                            titulo = "Distribución porcentual", 
                                            etiqueta_x = "Porcentaje", 
                                            etiqueta_y = "Categorías", 
...
# Ejemplo de uso
# Supongamos que `INDI30` es tu conjunto de datos y `Grainspad` es la variable categórica.
...
```{r 30_P1_Resumen_Binarios, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
# INDI30[, 23:30] <- INDI30[, 23:30] %>%
#   mutate(across(everything(), ~ as.numeric(. %in% c(1, "1"))))
#
# INDI30[, 33:39] <- INDI30[, 33:39] %>%
#   mutate(across(everything(), ~ as.numeric(. %in% c(1, "1"))))
library(dplyr)
...
# analizar_dicotomicas_numericas(data = INDI30,
...
# tabla_grafico1 <- analizar_dicotomicas_numericas(data = INDI30,
#         rango_indices = c(23:30),
#         titulo_tabla = "Tabla de Frecuencia de Incidencias",
#         titulo_grafico = "Porcentajes de Incidencias",
#         etiqueta_x = "Incidencias",
...
#
# # Ejecutar la función y guardar el resultado
# tabla_grafico2 <- analizar_dicotomicas_numericas(data = INDI30,
#         rango_indices = c(33:39),
#         titulo_tabla = "Resumen de Tratamientos",
#         titulo_grafico = "Porcentajes de Tratamiento",
#         etiqueta_x = "Tratamiento",
...
INDI30[ , 43:104] <- lapply(INDI30[ , 43:104], as.numeric)

# Llamar a la función y mostrar los resultados
LIMPION <- ayudin(dataframe = INDI30, columnas = c(43:105))
